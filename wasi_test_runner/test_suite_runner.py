    ...
    for test_path, name, config in all_tests:
        for filt in filters:
            ...
            test_case = _execute_single_test(
                runtime, meta, test_path, name, config, runtime.runner_class)
...
def _execute_single_test(
        ...
        runner_class: TestCaseRunnerBase = None
) -> TestCase:
    if runner_class is None:
    	runner_class = TestCaseRunner
    runner = runner_class(config, test_path, meta.wasi_version, runtime)
...
def _read_manifest(test_suite_path: Path) -> Manifest:
    ...
    if WasiVersion.contains(test_suite_path.name):
